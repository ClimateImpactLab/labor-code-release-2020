...
if "${fun_form}" == "splines" {
	...
	* I don't change polynomials!!!!!! 
	if "${interaction}" == "interacted" global ster_filename = "interacted_reg_1_factor_2025.ster"
...
program define generate_gammas_splines
	...
	* call this function in the common_functions.do to generate the globals 
	if "`interaction'" == "interacted" {
		generate_coef_spline_1factor `N' "`spline_varname'"
	}
	else if "`interaction'" == "uninteracted" {
...
* testing command:
generate_gammas_splines 3 "rcspl" "interacted"

* no update for polynomials
* generate varlists for the gammas
...
* for testing:
*write_csvv_header_splines 3 interacted 27_37_39 "${ster_filename}"
...
program define write_csvv_prednames_covarnames
    ...
    local covarnames_supplement
    forval i = 1/`N_clim_terms' {
        local covarnames_supplement "`covarnames_supplement' 1,"
    }
    forval i = 1/`N_clim_terms' {
        local covarnames_supplement "`covarnames_supplement' 1,"
    }
