#------------------------------------------------------------------------------------------
# Energy Delta Beta Over-Under Wrapper
...
#   to run delta betas for all regions to change in energy use (KWh/person) in a given year as a result
#   of climate change, splitting IR-level mortality predictions by days warmer than 20C and 
#   days below 20C. Can also be used to plot delta-beta curves.
#
#   To Run: depending on purpose, run through energy_db_script.R or make_curve_hist_plot.R
#------------------------------------------------------------------------------------------
...
pacman::p_load(ggplot2, 
               dplyr,
               readr)
source(glue('{REPO}/energy-code-release-2020/3_post_projection/0_utils/response_function.R'))
source(glue('{REPO}/post-projection-tools/response_function/yellow_purple_package.R'))

# 1. Delta Beta Functions ----------------------

standard_delta_beta = function(region, curve_ds, binclim, year, base_year, rebase_year, rnd.digits = 2, drop_zero_bins = T, rel.20 = T, full_db = F, bin=NULL) {
  ...
  # full db gives us results by 1 degree temp bin
  if (full_db == TRUE){
...
get_db_table = function(csvv, region, clim, covars, args) {
  ...
  covar_base_year = ifelse(base_year<2016, 2016, base_year) # gives previous year covar
  ...
  if(do_global){region='CAN.11.195.Rd6851092cdcff9c8'} # arbitrary region
  ...
  message('---full adapt')
  curve_ds = mapply_curve(csvv=csvv, covars=covars, year=years, regions=region, base_year=base_year, func=func,
                          TT_lower_bound=TT_lower_bound, TT_upper_bound=TT_upper_bound, TT_step=TT_step, 
                          config.path=config.path, model="TINV_clim", product='electricity')
  
  message('---no adapt')
  curve_ds = mapply_curve(csvv=csvv, covars=covars, year=years, regions=region, base_year=base_year, func=func, 
                          TT_lower_bound=TT_lower_bound, TT_upper_bound=TT_upper_bound, TT_step=TT_step,
                          curve_ds=curve_ds, adapt='no', 
                          config.path=config.path, model="TINV_clim", product='electricity')
  
  message('---income adapt')
  curve_ds = mapply_curve(csvv=csvv, covars=covars, year=years, regions=region, base_year=base_year, func=func,
                          TT_lower_bound=TT_lower_bound, TT_upper_bound=TT_upper_bound, TT_step=TT_step,
                          curve_ds=curve_ds, adapt='income', 
                          config.path=config.path, model="TINV_clim", product='electricity')
  
  message("got the curve")
  
  df = standard_delta_beta(region=region, curve_ds=curve_ds, binclim=binclim, year=years,
                           base_year=base_year, rebase_year=rebase_year, rnd.digits=5,
...
get_all_db_tables = function(region_list, args) {
  ...
  # Returns list
  df = lapply(region_list, get_db_table, csvv=csvv, clim=clim_df, covars=covars, args) %>%
    rbindlist()

  return(df)
}

save_sector_output = function(df, product, ssp, iam, rcp, gcm, year, out, global, full_db=F, rebased=T, slug=''){
  ...
  # Save out file
  file_name = glue("{gcm}-{rcp}-{ssp}-{iam}-delta_beta-{regions}-{year}-energy-{product}{slug}.csv")
  output_file = paste0(out, file_name)
