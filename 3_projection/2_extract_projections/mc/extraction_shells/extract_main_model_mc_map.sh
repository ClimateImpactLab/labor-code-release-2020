...
#   3. Change directories and edit the section marked "CHANGE HERE", especially extract_main_model_mc.yml.
#   4. Run:
...
output="/project/cil/gcp/outputs/labor/impacts-woodwork/montecarlo/test0109"
#results-root="/project/cil/gcp/outputs/labor/impacts-woodwork"
...
# - For aggregated aggregated="TRUE" & spatiallist=( aggregated ) 

show_output="FALSE" # if FALSE parallelizes using run_bg below
aggregated="TRUE" 
levels="FALSE"
varlist=( highriskimpacts ) # rebased highriskimpacts lowriskimpacts clip highrisk allrisk 
...
CONFIG=${REPO}/labor-code-release-2020/3_projection/2_extract_projections/mc/extraction_configs/extract_main_model_mc.yml
        ...
        for scn in "${scnlist[@]}"; do
          for suffix_aggregated in "${suffix_list[@]}"; do
            for yr in "${yearlist[@]}"; do
              ...
              echo "[DEBUG] Processing: ${ssp}_${rcp}_${iam}_${v}_${scn}${suffix_aggregated}_${yr}" >> "$LOG_FILE"
              echo "[DEBUG] basenames count: ${#clean_args[@]}" >> "$LOG_FILE"
              for i in "${!clean_args[@]}"; do
                printf '[DEBUG] basename[%d]=<%s>\n' "$i" "${clean_args[$i]}" >> "$LOG_FILE"
              done

              log="${LOG_DIR}/${ssp}_${rcp}_${iam}_${v}_${scn}${suffix_aggregated}_${yr}.log"
              ...
              python_args=(
                ...
                "--years=[${yr}]"
                "--yearsets=no"
                "--suffix=_${iam}_${v}_${scn}${suffix_aggregated}_${yr}_map"
                "--verbose"
              )
