...
processes=4 #set number of processes so that extraction will not use more than this 
...
config=/project/cil/home_dirs/egrenier/repos/labor-code-release-2020/3_projection/2_extract_projections/extraction_configs/extract_mc.yml
...
if [ "$levels" == "TRUE" ]; then
    suffix_list=( "-wage-levels" ) #"-pop-levels" "-gdp-levels" "-wage-levels" 
elif [ "$aggregated" == "TRUE" ]; then
    suffix_list=( "-wage-aggregated" )  #"-pop-aggregated" "-gdp-aggregated" "-wage-aggregated" 
else
    suffix_list=( "" )
fi
            ...
            for v in "${varlist[@]}" ; do
                for scn in "${scnlist[@]}" ; do
                    for suffix_aggregated in "${suffix_list[@]}" ; do
                        ...
                        out=${output}/${basename}/cloud #${rcp}/${iam}/${ssp}
                        mkdir -p ${out}
