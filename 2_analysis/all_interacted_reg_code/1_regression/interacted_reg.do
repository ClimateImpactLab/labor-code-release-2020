...
* 	1-factor (income, climate)
*	2-factor
*
* How to use:
* 1. Log in to a computing node.
* 2. Update "CHANGE HERE" settings in this file:
...
* Note on weights (slready coded):
* run regression 1_factor with mixed weights
...
foreach reg in $reg_list {
    local the_factor "${the_factor}"
    di "`reg_list'"
    
    use $dataset, clear
    drop if iso == "GBR" & year == 1984 & month == 1 & day == 2
    drop if inlist(iso, "MEX", "BRA")
    ...
    * Define treatment variables based on model type
    if "`reg'" == "1_factor" {
		if "${the_factor}" == "income" {
		* Low risk spline terms
		...
		* High risk × GDP interaction terms
		...
		gen tmax_rcspl_3kn_t0_hr_l    = tmax_rcspl_3kn_t0    * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v1 = tmax_rcspl_3kn_t0_v1 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v2 = tmax_rcspl_3kn_t0_v2 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v3 = tmax_rcspl_3kn_t0_v3 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v4 = tmax_rcspl_3kn_t0_v4 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v5 = tmax_rcspl_3kn_t0_v5 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t0_hr_l_v6 = tmax_rcspl_3kn_t0_v6 * risk_level * lr_tmax_p1

		gen tmax_rcspl_3kn_t1_hr_l    = tmax_rcspl_3kn_t1    * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v1 = tmax_rcspl_3kn_t1_v1 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v2 = tmax_rcspl_3kn_t1_v2 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v3 = tmax_rcspl_3kn_t1_v3 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v4 = tmax_rcspl_3kn_t1_v4 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v5 = tmax_rcspl_3kn_t1_v5 * risk_level * lr_tmax_p1
		gen tmax_rcspl_3kn_t1_hr_l_v6 = tmax_rcspl_3kn_t1_v6 * risk_level * lr_tmax_p1
		}
	...
	* Set the regression weight
	loc weight "rep_unit_year_sample_wgt"
	local ster_name "`reg_folder'/interacted_reg_`reg'_2026_noMEXBRA.ster"
    }
    else {
    ...
    * Interact each fixed effect with the risk binary
    ...
    local spec_desc "rcspline, 3 knots (27 28 41), tmax, differentiated treatment with lr interaction, fe = `fe', reg_type = `reg'"
    di "reghdfe mins_worked `reg_treatment' `reg_control' [pweight = `weight'], absorb(`reg_fe') vce(cl cluster_adm1yymm)"
    
    reghdfe mins_worked `reg_treatment' `reg_control' [pweight = `weight'], ///
        absorb(`reg_fe') vce(cl cluster_adm1yymm)
