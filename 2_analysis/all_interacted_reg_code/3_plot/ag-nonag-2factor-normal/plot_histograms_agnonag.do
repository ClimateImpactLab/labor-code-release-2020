...
program define plot_histograms
	...
	* Get cutoff values for each tercile from the grid file
	* The grid file has tercile = 1, 2, 3 with corresponding max values
	foreach x in loggdppc lrtmax {
		* Get max value for tercile 1 (upper bound of first tercile)
		sum max_`x' if tercile == 1, meanonly
		global `x'1 = `r(max)'
		
		* Get max value for tercile 2 (upper bound of second tercile)
		sum max_`x' if tercile == 2, meanonly
		global `x'2 = `r(max)'
		
		* Get max value for tercile 3 (upper bound of third tercile)
		sum max_`x' if tercile == 3, meanonly
		global `x'3 = `r(max)'
	}
	
	* Display cutoffs for verification
	...
	* Assign climate terciles based on cutoffs
	* Tercile 1: lowest values (lr_tmax_p1 <= cutoff1)
	* Tercile 2: middle values (cutoff1 < lr_tmax_p1 <= cutoff2)
	* Tercile 3: highest values (lr_tmax_p1 > cutoff2)
	...
	* Assign income terciles based on cutoffs
	* Tercile 1: poorest (log_gdp_pc_adm1 <= cutoff1)
	* Tercile 2: middle income (cutoff1 < log_gdp_pc_adm1 <= cutoff2)
	* Tercile 3: richest (log_gdp_pc_adm1 > cutoff2)
	...
	* Verify tercile assignments by country
	...
	* Check for missing tercile assignments
	...
	if r(N) > 0 {
		...
		di as error "Check if lr_tmax_p1 values are outside expected range"
	}
	count if missing(inc_t)
	if r(N) > 0 {
		di as error "WARNING: `r(N)' observations have missing inc_t"
		di as error "Check if log_gdp_pc_adm1 values are outside expected range"
	}
