******************************************************
* UNIFIED HISTOGRAM PLOTTING PROGRAM
* Supports: income, climate, and interacted interactions
******************************************************
...
program define plot_histograms
    ...
    * Get cutoff values from grid file
    ...
    * Assign climate terciles
    gen clim_t = 1 if lr_tmax_p1 <= $lrtmax1 & !missing(lr_tmax_p1)
    ...
    * Assign income terciles
    gen inc_t = 1 if log_gdp_pc_adm1 <= $loggdppc1 & !missing(log_gdp_pc_adm1)
    ...
    * Find unified y-axis maximum for percentages
    sum `weight'_hp, det
    local max_hp = `r(max)'
    sum `weight'_lp, det
    local max_lp = `r(max)'
    ...
    * Set y-axis for absolute counts
    if "`weight'" == "no_wgt" {
        sum `weight', det
    ...
    ******************************************************
    * Plot based on interaction type
    ******************************************************
    
    * CLIMATE INTERACTION (3 plots)
    ...
    * INCOME INTERACTION (3 plots)
        ...
        forvalues inc=1(1)3 {
            ...
            if r(N) > 0 {
                local opts_x `"xlab(${temp_min}(10)${temp_max}, labs(medium)) xsc(range(${temp_min} ${temp_max})) xtitle("")"'
                local opts_abs `"barwidth(0.09) ylab(#5, labs(medium) angle(vertical) labcolor(white) tlcolor(white) nogrid) graphregion(color(white) margin(zero)) plotregion(margin(zero)) fysize(20) `opts_x'"'
                local opts_pct `"barwidth(0.09) ylab(#5, labs(medium) angle(vertical) labcolor(white) tlcolor(white) nogrid) graphregion(color(white) margin(zero)) plotregion(margin(zero)) fysize(20) `opts_x'"'
                
                * Absolute count histograms
                tw bar `weight'_high temp if inc_t == `inc', ///
                    `opts_abs' ysc(range(`range_abs') lcolor(black)) ytitle("Obs", size(medium) color(white)) ///
                    name(hist`i'_hl_abs, replace)
                ...
                * Percentage histograms
                tw bar `weight'_hp temp if inc_t == `inc', ///
                    `opts_pct' ysc(range(0 `ymax_pct') lcolor(black) lw(medium)) ///
                    xsc(lw(medium)) ///
                    ytitle("{bf:%}", size(medium) color(white)) ///
                    name(hist`i'_hl_pct, replace)
    ...
    * INTERACTED (CLIMATE X INCOME) - 9 plots
        ...
        forvalues inc=1(1)3 {
            forvalues clim=1(1)3 {
                ...
                if r(N) > 0 {
                    local opts_x `"xlab(${temp_min}(10)${temp_max}, labs(small)) xsc(range(${temp_min} ${temp_max})) xtitle("")"'
                    local opts_abs `"barwidth(0.09) ylab(#4, labs(medium) angle(vertical)) graphregion(color(white) margin(zero)) plotregion(margin(zero)) fysize(10) `opts_x'"'
                    local opts_pct `"barwidth(0.09) ylab(#4, labs(medium) angle(vertical)) graphregion(color(white) margin(zero)) plotregion(margin(zero)) fysize(10) `opts_x'"'
                    
                    * Absolute count histograms
                    tw bar `weight'_high temp if clim_t == `clim' & inc_t == `inc', ///
                        `opts_abs' ysc(range(`range_abs')) ytitle("Obs", size(medium)) ///
                        name(hist`i'_hl_abs, replace)
                    ...
                    * Percentage histograms
                    tw bar `weight'_hp temp if clim_t == `clim' & inc_t == `inc', ///
                        `opts_pct' ysc(range(0 `ymax_pct')) ytitle("%", size(medium)) ///
                        name(hist`i'_hl_pct, replace)
    ...
    ******************************************************
    * Calculate counts for plot subtitles
    ******************************************************
    di "STARTING COUNTS..."
