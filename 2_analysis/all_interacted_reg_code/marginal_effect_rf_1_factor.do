		...
		* run code based on the model
		if "$reg_list" == "1_factor" {
			...
			* collect spline terms in macros
			collect_mix_spline_terms, splines(0 1) unint(unint) int_gdp(int_gdp)

			* need this blank variable to get standard errors in predictnl
			gen mins_worked = .

			* predict gdp marginal response function by risk
				predictnl yhat_low =	(T_spline0 - ref_spline0) * (${unint0}) + (T_spline1 - ref_spline1) * (${unint1}), ci(lowerci_low upperci_low) se(se_low)

				predictnl yhat_high =	(T_spline0 - ref_spline0) * (${int_gdp0}) +	(T_spline1 - ref_spline1) * (${int_gdp1}), ci(lowerci_high upperci_high) se(se_high)
		}
