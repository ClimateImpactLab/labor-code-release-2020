...
foreach reg in $reg_list {
    ...
    if "`reg'" == "1_factor" {
        loc ster_name "interacted_reg_1_factor_2026_272841_income"
    }
    if "`reg'" == "2_factor" {
        loc ster_name "interacted_reg_2_factor_2025_climatehigh_only"
    }

    test_interaction_spline `f' `N_knots' `data_subset' "`reg'" "`ster_name'" rep_unit_year_sample_wgt gdp
}
