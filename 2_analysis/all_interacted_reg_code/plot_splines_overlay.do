...
log using "/project/cil/home_dirs/maiqi/repos/labor-code-release-2020/logs/risk_overlay.smcl", replace
clear all 
...
generate_coef_spline_1factor 3 rcspl //no 1factor
		...
		foreach weight in $weights_list {
			foreach f in ${fe_list} {
				foreach data_subset in ${data_subset_list} {
					...
					plot_interacted_spline_overlay $interaction `f' `p' `data_subset' all_data_with_ci `ster_name'
				}
			}
		}
	}	
