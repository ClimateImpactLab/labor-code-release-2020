...
program define gen_plot 
	...
	local range_hr = "-50 80" //original marginal
...
//       global b_T_x_gdp_spline_`k'_lr 0
        ...
        global b_T_spline_`k'_hl 0
        global b_T_x_gdp_spline_`k'_hl 0

        global b_T_spline_`k'_hr 0
        global b_T_x_gdp_spline_`k'_hr 0

        global b_T_spline_`k'_lr _b[tmax_`spl_varname'_`N_knots'kn_t`k'_lr]
        global b_T_spline_`k'_hl _b[tmax_`spl_varname'_`N_knots'kn_t`k'_hr]
        global b_T_x_gdp_spline_`k'_hl _b[tmax_`spl_varname'_`N_knots'kn_t`k'_hr_g]

        forval lag = 1/6 {
            global b_T_spline_`k'_lr ${b_T_spline_`k'_lr}+_b[tmax_`spl_varname'_`N_knots'kn_t`k'_lr_v`lag']
            global b_T_spline_`k'_hl ${b_T_spline_`k'_hl}+_b[tmax_`spl_varname'_`N_knots'kn_t`k'_hr_v`lag']
            global b_T_x_gdp_spline_`k'_hl ${b_T_x_gdp_spline_`k'_hl}+_b[tmax_`spl_varname'_`N_knots'kn_t`k'_hr_g_v`lag']
        }

        global b_T_spline_`k'_hr        ${b_T_spline_`k'_hl} - ${b_T_spline_`k'_lr}
        global b_T_x_gdp_spline_`k'_hr  ${b_T_x_gdp_spline_`k'_hl} - ${b_T_x_gdp_spline_`k'_lr}
    }
end
	...
	* lr: low risk, hr: high risk (risk interaction terms only), hl: lr + hr = response of high risk 
 	foreach risk in _hl _lr _hr {
 		...
 		* add in the histogram subplots
 		forval i=1(1)$max_g {
 			local h`i' = "/project/cil/home_dirs/`c(username)'/repos/labor-code-release-2020/output/interacted_reg_output/plots/histograms/$interaction/hist`i'`risk'_`hist_style'.gph"
 		}
 		
 		if "`interaction'" != "income" {
 			graph combine plot7 plot8 plot9 "`h7'" "`h8'" "`h9'" plot4 plot5 plot6 "`h4'" "`h5'" "`h6'" plot1 plot2 plot3 "`h1'" "`h2'" "`h3'", plotregion(color(white)) graphregion(color(white) margin(t=5 b=5)) xcomm imargin(0 0 0 0) cols(3) title("`plot_tag'") subtitle("`ster_name', hist: `hist_weight' `hist_style'", size(vsmall))
 		}
 		else {
 			graph combine plot1 plot2 plot3 "`h1'" "`h2'" "`h3'", plotregion(color(white)) graphregion(color(white) margin(t=5 b=5)) xcomm imargin(0 0 0 0) cols(3) title("`plot_tag'") subtitle("`ster_name', hist: `hist_weight' `hist_style'", size(vsmall))
 		}
 	...
 	if "`interaction'" == "triple_int" {
		...
		foreach marginal_var in lrtmax gdp {
			* plot marginal effect of lrtmax and log gdp per capita
			foreach risk in _lr _hr {

				forval g = 1/3 {

					gen_varying_marginal_resp_spline `N_knots' `marginal_var' `risk' `g'
					di "PLOTTING `g' for `marginal_var' and ${other_var}`g' == ${$other_var}`g'}"
					gen_marg_plot "${other_var}`g'" `marginal_var'`g' `plot_style' 

				}

				graph combine `marginal_var'1 `marginal_var'2 `marginal_var'3, plotregion(color(white)) graphregion(color(white)) cols(3) ycommon title("`marginal_var'_`risk'") subtitle("`ster_name'")
				graph export "`ster_name'_marginal_`marginal_var'`risk'.pdf", replace
	 	...
	 	if "`interaction'" == "income" loc marg_vars "gdp"
	 	else loc marg_vars = "gdp lrtmax"
		foreach marginal_var in `marg_vars' {
			* plot marginal effect of lrtmax and log gdp per capita
