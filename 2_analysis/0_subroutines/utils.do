...
program define init 
	...
	*cap ssc install parallel
	...
	if "`c(hostname)'" == "battuta" {
		global shares_path "/project/cil/battuta_shares"
	}
	else global shares_path "/project/cil/battuta_shares"
	*local path `c(pwd)'
...
program define run_specification
	...
	if "`diff_treat'" == "diff_treat"{
		local treatment (`treatment')##i.risk_level
	}

	if "`diff_cont'" == "diff_cont"{
		local control (`control')##i.risk_level
		local fe 
		foreach f in ${`fe_ver'} {
			local fe `fe' `f'#risk_level
		}
	}
...
* 3x3 plot layout and tercile means, with 3 risk groups: low / ag / nonag
cap program drop generate_grids_sector
program define generate_grids_sector
    ...
    * ===== 1. CLIMATE TERCILES (mean_lrtmax) =====

    if "`tercile'" == "hierid" {
        use "/project/cil/norgay/CIL_labor/2_regression/time_use/input/lrtmax_grid.dta", clear
        * long-run tmax from CCSM4 rcp8.5, averaged 1950–2005
    }
    else if ("`tercile'" == "rep_unit") {
        use "/project/cil/norgay/CIL_labor/2_regression/time_use/input/rep_unit_terciles_grid.dta", clear
        * generated by time_use/tercile_by_rep_units.do
    }
    else {
        di "!! Incorrect tercile specification. Permitted: rep_unit, hierid."
        exit 198
    }

    summarize mean_lrtmax, detail
    ...
    * ===== 2. INCOME TERCILES (mean_loggdppc) =====

    if "`tercile'" == "hierid" {
        use "/project/cil/norgay/CIL_labor/2_regression/time_use/input/loggdppc_2010_grid.dta", clear
        * income at impact-region level
    }
    else if ("`tercile'" == "rep_unit") {
        use "/project/cil/norgay/CIL_labor/2_regression/time_use/input/rep_unit_terciles_grid.dta", clear
    }

    summarize mean_loggdppc, detail
    ...
    if ("`interaction'" == "interacted" | "`interaction'" == "triple_int") {
        ...
        * labels for each of the 9 panels
    ...
    * ===== 3. LOAD COUNTS CREATED BY plot_histograms =====

    use "${ROOT_INT_DATA}/xtiles/`tercile'_terciles_count.dta", clear

    * This file must contain:
    *   clim    inc
    *   count_low   count_ag   count_nonag
    *   count_rep_unit   count_rep_year
    ...
    if ("`interaction'" == "interacted" | "`interaction'" == "triple_int") {
        ...
        forvalues inc = 1/3 {
            forvalues clim = 1/3 {
                ...
                * rep-unit counts (same across risk groups)
                quietly summarize count_rep_unit if clim == `clim' & inc == `inc'
                if (r(N) != 0) global ru_plot`i' = r(mean)
                else          global ru_plot`i' = 0

                quietly summarize count_rep_year if clim == `clim' & inc == `inc'
                if (r(N) != 0) global ry_plot`i' = r(mean)
                else          global ry_plot`i' = 0

                * risk-specific counts
                quietly summarize count_low if clim == `clim' & inc == `inc'
                if (r(N) != 0) global risk_low_plot`i' = r(mean)
                else          global risk_low_plot`i' = 0

                quietly summarize count_ag if clim == `clim' & inc == `inc'
                if (r(N) != 0) global risk_ag_plot`i' = r(mean)
                else          global risk_ag_plot`i' = 0

                quietly summarize count_nonag if clim == `clim' & inc == `inc'
                if (r(N) != 0) global risk_nonag_plot`i' = r(mean)
                else          global risk_nonag_plot`i' = 0

                * total N across all three risk groups
                global risk_all_plot`i' = ${risk_low_plot`i'} ///
                                          + ${risk_ag_plot`i'}  ///
                                          + ${risk_nonag_plot`i'}
        ...
        di "INTERACTION `interaction' getting counts"

        local i = 1
        forvalues inc = 1/3 {

            quietly summarize count_rep_unit if inc == `inc'
            if (r(N) != 0) global ru_plot`i' = r(sum)
            else          global ru_plot`i' = 0

            quietly summarize count_rep_year if inc == `inc'
            if (r(N) != 0) global ry_plot`i' = r(sum)
            else          global ry_plot`i' = 0

            quietly summarize count_low if inc == `inc'
            if (r(N) != 0) global risk_low_plot`i' = r(sum)
            else          global risk_low_plot`i' = 0

            quietly summarize count_ag if inc == `inc'
            if (r(N) != 0) global risk_ag_plot`i' = r(sum)
            else          global risk_ag_plot`i' = 0

            quietly summarize count_nonag if inc == `inc'
            if (r(N) != 0) global risk_nonag_plot`i' = r(sum)
            else          global risk_nonag_plot`i' = 0
...
program define generate_grids
	...
	* add counts of observations, rep-units, and rep-unit-years.
	use "/project/cil/norgay/CIL_labor/2_regression/time_use/input/`tercile'_terciles_count.dta", clear
	...
	if "`interaction'" == "interacted" | "`interaction'" == "triple_int" {
		...
		forvalues inc=1(1)3 {
			forvalues clim=1(1)3 {
				...
				* risk-specific counts 
				sum count_low if clim == `clim' & inc == `inc'
				if (`r(N)' != 0) gl risk_low_plot`i' = `r(mean)' 
				else gl risk_low_plot`i' = 0

				sum count_ag if clim == `clim' & inc == `inc'
				if (`r(N)' != 0) gl risk_ag_plot`i' = `r(mean)' 
				else gl risk_ag_plot`i' = 0
				
				sum count_nonag if clim == `clim' & inc == `inc'
				if (`r(N)' != 0) gl risk_nonag_plot`i' = `r(mean)' 
				else gl risk_nonag_plot`i' = 0
...
program define gen_plot
    ...
    local range_all = "-60 100"

    local p1 = "${p1`g'`risk'_`weight'}"
    local p5 = "${p5`g'`risk'_`weight'}"
    local p95 = "${p95`g'`risk'_`weight'}"
    local p99 = "${p99`g'`risk'_`weight'}"

    local cutoffs ""
    if "`p1'" != "" & "`p5'" != "" & "`p95'" != "" & "`p99'" != "" {
        local cutoffs "xline(`p1', lcol(gold) lpatt(-)) xline(`p99', lcol(gold) lpatt(-)) xline(`p5', lcol(orange) lpatt(_)) xline(`p95', lcol(orange) lpatt(_))"
    }

    preserve
    if "`plot_style'" == "all_data_with_ci" {
        twoway ///
            rarea upper_ci lower_ci temp, col(ltbluishgray) || ///
            ...
            subtitle("${risk_`risk'_`plot_name'} obs, ${ru_`plot_name'} rep-units, ${ry_`plot_name'} rep-unit-years.", size(small)) ///
            yscale(range(`range_all')) ///
            xsc(range(-20 50))
    }
...
program define gen_marg_plot 
	...
	if "`plot_style'" == "all_data_no_ci"{
		tw  line yhat temp, lc (dknavy) yline(0) title("`plot_title'") legend(off) graphregion(color(white)) ylabel(,angle(horizontal)) ytitle("mins worked") ysize(3)
 xtitle("Temperature C", height(6)) name(`plot_name', replace)
	}
...
* generate spline coefficients for plotting (3 risk groups: low / ag / nonag)
cap program drop generate_coef_spline_sector
program define generate_coef_spline_sector

    args N_knots spl_varname

    * N_new_vars = number of spline terms = N_knots - 2
    local N_new_vars = `N_knots' - 2
    di "Number of spline terms (per temperature): `N_new_vars'"

    forvalues k = 0/`N_new_vars' {

        * initialize globals for each risk group
        * low  : baseline (risk_level == 0)
        * ag   : baseline + 1.risk_level
        * nonag: baseline + 2.risk_level

        global b_T_spline_`k'_low 0
        global b_T_x_gdp_spline_`k'_low 0
        global b_T_x_lrtmax_spline_`k'_low 0

        global b_T_spline_`k'_ag 0
        global b_T_x_gdp_spline_`k'_ag 0
        global b_T_x_lrtmax_spline_`k'_ag 0

        global b_T_spline_`k'_nonag 0
        global b_T_x_gdp_spline_`k'_nonag 0
        global b_T_x_lrtmax_spline_`k'_nonag 0

        * internal base components (common to all risk groups)
        global b_T_spline_`k'_base 0
        global b_T_x_gdp_spline_`k'_base 0
        global b_T_x_lrtmax_spline_`k'_base 0

        * internal deviations for ag (risk_level == 1)
        global b_T_spline_`k'_ag_dev 0
        global b_T_x_gdp_spline_`k'_ag_dev 0
        global b_T_x_lrtmax_spline_`k'_ag_dev 0

        * internal deviations for nonag (risk_level == 2)
        global b_T_spline_`k'_nonag_dev 0
        global b_T_x_gdp_spline_`k'_nonag_dev 0
        global b_T_x_lrtmax_spline_`k'_nonag_dev 0

        * base spline coefficient for temperature
        global b_T_spline_`k'_base = _b[tmax_`spl_varname'_`N_knots'kn_t`k']
        global b_T_x_gdp_spline_`k'_base = _b[c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.log_gdp_pc_adm1]
        global b_T_x_lrtmax_spline_`k'_base = _b[c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.lr_tmax_p1]

        * deviations for ag (1.risk_level) and nonag (2.risk_level)
        global b_T_spline_`k'_ag_dev = _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k']
        global b_T_x_gdp_spline_`k'_ag_dev = _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.log_gdp_pc_adm1]
        global b_T_x_lrtmax_spline_`k'_ag_dev = _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.lr_tmax_p1]

        global b_T_spline_`k'_nonag_dev = _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k']
        global b_T_x_gdp_spline_`k'_nonag_dev = _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.log_gdp_pc_adm1]
        global b_T_x_lrtmax_spline_`k'_nonag_dev = _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'#c.lr_tmax_p1]

        * add v1 to v6 lagged terms to base and deviations
        forvalues lag = 1/6 {

            global b_T_spline_`k'_base = ${b_T_spline_`k'_base} + ///
                _b[tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag']
            global b_T_x_gdp_spline_`k'_base = ${b_T_x_gdp_spline_`k'_base} + ///
                _b[c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.log_gdp_pc_adm1]
            global b_T_x_lrtmax_spline_`k'_base = ${b_T_x_lrtmax_spline_`k'_base} + ///
                _b[c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.lr_tmax_p1]

            global b_T_spline_`k'_ag_dev = ${b_T_spline_`k'_ag_dev} + ///
                _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag']
            global b_T_x_gdp_spline_`k'_ag_dev = ${b_T_x_gdp_spline_`k'_ag_dev} + ///
                _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.log_gdp_pc_adm1]
            global b_T_x_lrtmax_spline_`k'_ag_dev = ${b_T_x_lrtmax_spline_`k'_ag_dev} + ///
                _b[1.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.lr_tmax_p1]

            global b_T_spline_`k'_nonag_dev = ${b_T_spline_`k'_nonag_dev} + ///
                _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag']
            global b_T_x_gdp_spline_`k'_nonag_dev = ${b_T_x_gdp_spline_`k'_nonag_dev} + ///
                _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.log_gdp_pc_adm1]
            global b_T_x_lrtmax_spline_`k'_nonag_dev = ${b_T_x_lrtmax_spline_`k'_nonag_dev} + ///
                _b[2.risk_level#c.tmax_`spl_varname'_`N_knots'kn_t`k'_v`lag'#c.lr_tmax_p1]
        }

        * now construct coefficients for each risk group

        * low-risk group (baseline only)
        global b_T_spline_`k'_low = ${b_T_spline_`k'_base}
        global b_T_x_gdp_spline_`k'_low = ${b_T_x_gdp_spline_`k'_base}
        global b_T_x_lrtmax_spline_`k'_low = ${b_T_x_lrtmax_spline_`k'_base}

        * high-risk agriculture (baseline + 1.risk_level deviation)
        global b_T_spline_`k'_ag = ${b_T_spline_`k'_base} + ${b_T_spline_`k'_ag_dev}
        global b_T_x_gdp_spline_`k'_ag = ${b_T_x_gdp_spline_`k'_base} + ${b_T_x_gdp_spline_`k'_ag_dev}
        global b_T_x_lrtmax_spline_`k'_ag = ${b_T_x_lrtmax_spline_`k'_base} + ${b_T_x_lrtmax_spline_`k'_ag_dev}

        * high-risk non-agriculture (baseline + 2.risk_level deviation)
        global b_T_spline_`k'_nonag = ${b_T_spline_`k'_base} + ${b_T_spline_`k'_nonag_dev}
        global b_T_x_gdp_spline_`k'_nonag = ${b_T_x_gdp_spline_`k'_base} + ${b_T_x_gdp_spline_`k'_nonag_dev}
...
program define generate_coef_spline_interacted
	...
	global b_T_x_lrtmax_spline_`k'_lr 0
        ...
        forval lag = 1/6 {
            ...
            global b_T_x_gdp_spline_`k'_hr ${b_T_x_gdp_spline_`k'_hr} + ///
                _b[tmax_`spl_varname'_`N_knots'kn_t`k'_hr_g_v`lag']
        }
...
program define generate_coef_polynomials
	...
	* compute coefficients for each variable at each polylnomial 
	...
	forval degree = 1/4{
		global b_temp_`degree'_hr _b[1.risk_level#c.tmax_p`degree'] 
		global b_temp_gdp_`degree'_hr _b[1.risk_level#c.tmax_p`degree'#c.log_gdp_pc_adm1]
		global b_temp_lrtmax_`degree'_hr _b[1.risk_level#c.tmax_p`degree'#c.lr_tmax_p1]

		forval lag = 1/6 {
			global b_temp_`degree'_hr ${b_temp_`degree'_hr}+_b[1.risk_level#c.tmax_p`degree'_v`lag']		
			global b_temp_gdp_`degree'_hr ${b_temp_gdp_`degree'_hr}+_b[1.risk_level#c.tmax_p`degree'_v`lag'#c.log_gdp_pc_adm1]
			global b_temp_lrtmax_`degree'_hr ${b_temp_lrtmax_`degree'_hr}+_b[1.risk_level#c.tmax_p`degree'_v`lag'#c.lr_tmax_p1]
		}
	}
...
program define test_interaction_spline
    ...
    if "`reg'" == "2_factor" {
        foreach risk in lr hl {
            ...
            forval i = 0/`N_new_vars' {
                local test_gdp    `test_gdp'    ( ${b_T_x_gdp_spline_`i'_`risk'}    = 0 )
                local test_lrtmax `test_lrtmax' ( ${b_T_x_lrtmax_spline_`i'_`risk'} = 0 )
                local test_joint  `test_joint'  ( ${b_T_x_gdp_spline_`i'_`risk'}    = 0 ) ( ${b_T_x_lrtmax_spline_`i'_`risk'} = 0 )
            }
    ...
    * Set matrix names
    ...
    putexcel set "${rf_folder}/test_spline_results_`wgt'_`reg'.xlsx", sheet("results") replace
...
program define gen_response_surface_spline
    ...
    local response_surface "0"
    
    forval i = 0/`N_new_vars' {
        local response_surface "`response_surface' + (${b_T_spline_`i'_`risk'}) * (T_spline`i' - T_ref_spline`i')"
        local response_surface "`response_surface' + (${b_T_x_gdp_spline_`i'_`risk'}) * (T_spline`i' - T_ref_spline`i') * ${minc`grid'}"
        if "$interaction" == "interacted" | "$interaction" == "triple_int" {
            local response_surface "`response_surface' + (${b_T_x_lrtmax_spline_`i'_`risk'}) * (T_spline`i' - T_ref_spline`i') * ${lrtmax`grid'}"
        }

        if "$interaction" == "triple_int" {
            local response_surface "`response_surface' + (${b_T_x_gdp_x_lrtmax_spline_`i'_`risk'}) * (T_spline`i' - T_ref_spline`i') * ${minc`grid'} * ${lrtmax`grid'}"
        }
    }
    
    cap drop yhat upper_ci lower_ci
    di "`risk' `grid': `response_surface'"

    capture predictnl yhat = `response_surface', ci(lower_ci upper_ci)
    if _rc != 0 {
        di as error "Error in predictnl command"
        di as error "Expression: `response_surface'"
        error _rc
    }
end
