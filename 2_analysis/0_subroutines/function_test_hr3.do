...
program define make_spline_terms
    ...
    global knots = "`knot1' `knot2' `knot3'"

    local scaling_factor = (`knot3' - `knot1')^2
    
    mkspline temp_spl = temp, cubic knots($knots)
    mkspline ref_spl = ref, cubic knots($knots)

    di "matching these splines with the transformed-after-aggregated data"

    quietly replace temp_spl2 = temp_spl2 * `scaling_factor'
    quietly replace ref_spl2 = ref_spl2 * `scaling_factor'

    // temp_spl1 -> tmax_rcspl_3kn_t0
    // temp_spl2 -> tmax_rcspl_3kn_t1
    rename temp_spl1 tmax_rcspl_3kn_t0
    rename temp_spl2 tmax_rcspl_3kn_t1
    
    gen T_spline0 = tmax_rcspl_3kn_t0
    gen T_spline1 = tmax_rcspl_3kn_t1
    rename ref_spl1 ref_spline0
    rename ref_spl2 ref_spline1
end
...
prog def collect_spline_terms
	...
	foreach i in `splines' {
		...
		gl `int'`i' = "
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'] + 
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v1] + 
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v2] + 
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v3] + 
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v4] + 
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v5] +
			_b[1.high_risk3#c.tmax_rcspl_3kn_t`i'_v6] "
		;
...
prog def collect_polynomial_terms
	...
	forval degree=1(1)`order' {
		...
		gl `int'`degree' = 
			"_b[1.high_risk3#c.tmax_p`degree'] +" + 
			"_b[1.high_risk3#c.tmax_p`degree'_v1] +" + 
			"_b[1.high_risk3#c.tmax_p`degree'_v2] +" +
			"_b[1.high_risk3#c.tmax_p`degree'_v3] +" + 
			"_b[1.high_risk3#c.tmax_p`degree'_v4] +" +
			"_b[1.high_risk3#c.tmax_p`degree'_v5] +" +
			"_b[1.high_risk3#c.tmax_p`degree'_v6] "
		;
	...
	* collect the coefficients for each bin
	foreach bin in $`bins' {
		...
		* add prefix for numerical bins
		...
		gl `int'`bin' = 
			"_b[1.high_risk3#c.`coef'] +" + 
			"_b[1.high_risk3#c.`coef'_v1] +" + 
			"_b[1.high_risk3#c.`coef'_v2] +" + 
			"_b[1.high_risk3#c.`coef'_v3] +" + 
			"_b[1.high_risk3#c.`coef'_v4] +" + 
			"_b[1.high_risk3#c.`coef'_v5] +" + 
			"_b[1.high_risk3#c.`coef'_v6] "
		;
