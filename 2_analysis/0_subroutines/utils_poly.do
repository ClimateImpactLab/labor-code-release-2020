...
program define plot_interacted_poly
    ...
    * --------------------------------------------------
    foreach risk in _lr _hr _hl {
        ...
        forval i = 1/$max_g {
            local h`i' = ///
	    "${DIR_OUTPUT}/interacted_reg_output/2_factor_poly/histograms/hist`i'`risk'_`hist_style'.gph"
        }
        ...
        if "`interaction'" != "income" {
            graph combine ///
                ...
                ycomm xcomm imargin(0 0 0 0) cols(3) ///
                title("`plot_tag'") ///
...
program define gen_plot 
    ...
    * --------------------------------------------------
    if "`plot_style'" == "all_data_with_ci"{
        if "`extrema_note'" != "" {
            tw rarea upper_ci lower_ci temp temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'} col(ltbluishgray) || ///
               line yhat temp temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'}, lc(dknavy) yline(0) `peak_lines' ///
...
// 	       ysc(r(${YMIN`risk'} ${YMAX`risk'}))
            ...
            tw rarea upper_ci lower_ci temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'}, col(ltbluishgray) || ///
               line yhat temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'}, lc(dknavy) yline(0) ///
               title("`plot_title'") legend(off) ///
               ylab(#8,labs(vsmall) ang(vertical)) ///
               ytitle("mins worked", size(small)) ///
               xlab("",labs(small)) fysize(65) xtitle("") ///
...
// 	       ysc(r(${YMIN`risk'} ${YMAX`risk'})) 
    ...
    if "`plot_style'" == "all_data_no_ci"{
        if "`extrema_note'" != "" {
            tw line yhat temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'}, lc(dknavy) yline(0) `peak_lines' ///
               title("`plot_title'") legend(off) ///
               ylab(#8,labs(vsmall) ang(vertical)) ///
               ytitle("mins worked", size(small)) ///
               xlab("",labs(small)) fysize(65) xtitle("") ///
...
// 	       ysc(r(${YMIN`risk'} ${YMAX`risk'})) 
            ...
            tw line yhat temp if temp>=${p1`g'`risk'_`weight'} & temp<=${p99`g'`risk'_`weight'}, lc(dknavy) yline(0) ///
               title("`plot_title'") legend(off) ///
               ylab(#8,labs(vsmall) ang(vertical)) ///
               ytitle("mins worked", size(small)) ///
               xlab("",labs(small)) fysize(65) xtitle("") ///
