...
program define make_spline_terms
    ...
    gen T_spline0 = temp_spl1
    gen T_spline1 = temp_spl2
    gen ref_spline0 = ref_spl1
    gen ref_spline1 = ref_spl2
    
    rename temp_spl1 tmax_rcspl_3kn_t0
    rename temp_spl2 tmax_rcspl_3kn_t1
    
    drop ref_spl1 ref_spl2
end
...
prog def collect_spline_terms
	...
	foreach i in `splines' {
		...
		gl `int'`i' = "
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'] + 
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v1] + 
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v2] + 
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v3] + 
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v4] + 
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v5] +
			_b[1.manuf#c.tmax_rcspl_3kn_t`i'_v6] "
		;
...
prog def collect_polynomial_terms
	...
	forval degree=1(1)`order' {
		...
		gl `int'`degree' = 
			"_b[1.manuf#c.tmax_p`degree'] +" + 
			"_b[1.manuf#c.tmax_p`degree'_v1] +" + 
			"_b[1.manuf#c.tmax_p`degree'_v2] +" +
			"_b[1.manuf#c.tmax_p`degree'_v3] +" + 
			"_b[1.manuf#c.tmax_p`degree'_v4] +" +
			"_b[1.manuf#c.tmax_p`degree'_v5] +" +
			"_b[1.manuf#c.tmax_p`degree'_v6] "
		;
	...
	* collect the coefficients for each bin
	foreach bin in $`bins' {
		...
		* add prefix for numerical bins
		...
		gl `int'`bin' = 
			"_b[1.manuf#c.`coef'] +" + 
			"_b[1.manuf#c.`coef'_v1] +" + 
			"_b[1.manuf#c.`coef'_v2] +" + 
			"_b[1.manuf#c.`coef'_v3] +" + 
			"_b[1.manuf#c.`coef'_v4] +" + 
			"_b[1.manuf#c.`coef'_v5] +" + 
			"_b[1.manuf#c.`coef'_v6] "
		;
