...
import delim "`rf_folder'/uninteracted_reg_comlohi_table_values_2025.csv", clear
...
foreach reg in by_risk common {
	...
	loc R2_`reg'	= round(`e(r2_a)', 0.01)

}

convert_table, categories("comm low high marg")		///
	r2(`R2_common' `R2_by_risk' `R2_by_risk' `R2_by_risk')		///
	n(`N_common' `N_low' `N_high' `N_common')
