...
outname <- "uninteracted_polynomials"
format  <- "pdf"
...
# Response function
rf <- read_csv(glue(
  "{DIR_RF}/uninteracted_polynomials/",
  "uninteracted_polynomials_nochn_4_full_response_2025_oldold.csv"
))
...
# only two groups: low, high
risk_levels <- c("low", "high")
...
# Histogram with risk-adjusted weights (for low/high):
hist_adj_wgt <- mclapply(
  risk_levels,
  df   = temp_dist,
  vars = c("risk_adj_sample_wgt"),
  FUN  = reshape,
  mc.cores = n_cols
) %>%
  rbindlist(use.names = TRUE) %>%
  mutate(weight = risk_adj_sample_wgt)
...
plot_temp_panels <- function(data,
  ...
  grid_plot <- (col_headers / strips) +
    plot_layout(heights = c(0.10, 0.90))
  
  titled <- grid_plot +
    plot_annotation(
      title = "Global Labor Supply–Temperature Response (Poly2) - High Risk = Old",
      theme = theme(
...
save_dir <- "/project/cil/home_dirs/maiqi/repos/labor-code-release-2020/output/rf_plots/uninteracted_reg_poly_2025"
dir.create(save_dir, showWarnings = FALSE, recursive = TRUE)

p <- plot_temp_panels(
  data, hist_no_wgt, hist_adj_wgt,
  use_adj = TRUE,
  risk_levels = c("low", "high")
)

outfile <- glue("{save_dir}/uninteracted_reg_poly4old.pdf")
