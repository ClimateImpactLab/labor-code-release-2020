...
outname <- "uninteracted_reg_comlohi"
format  <- "pdf"
...
rf <- read_csv(glue(
  ...
  "uninteracted_reg_comlohi_full_response_2025_high_old.csv"
))

# Temperature distribution and densities (global)
temp_dist <- read_csv(
  glue("{DIR_OUTPUT}/temp_dist/Dec2025/global_temp_dist_old.csv")
)
...
# only three groups: comm, low, high
...
hist_adj_wgt <- mclapply(
  risk_levels,
  df   = temp_dist,
  vars = c("risk_adj_sample_wgt_old", "pop_adj_sample_wgt"),
  FUN  = reshape,
  mc.cores = n_cols
) %>%
  rbindlist(use.names = TRUE) %>%
...
plot_temp_panels <- function(data,
  ...
  titled <- grid +
    plot_annotation(
      title = "Global Labor Supply–Temperature Response - High Risk = Old",
      theme = theme(
...
save_dir <- "/project/cil/home_dirs/maiqi/repos/labor-code-release-2020/output/rf_plots/uninteracted_reg_comlohi_2025_old"
...
outfile <- glue("{save_dir}/uninteracted_reg_comlohi_rf_plot_2025_old.pdf")
