...
################################################################################
# uninteracted_reg_comlohi_rf_figure3.R
#
# PURPOSE:
#   Plot temperature response functions for:
#     - All workers (comm)
#     - Low-risk workers (low)
#     - High-risk workers (high)
#
#   Each panel includes:
...
#
# OUTPUT:
#   RF panels (A/B/C) with aligned histograms and shared temperature axis
################################################################################
...
outname <- "uninteracted_reg_comlohi_figure3"
format  <- "pdf"
...
plot_CI_beyond_cap <- "YES"   # "YES" (default) or "NO"
...
# temperature distribution + densities
temp_dist <- read_csv(
  glue("{DIR_OUTPUT}/temp_dist/temp_dist.csv")
)
...
if (FRACTION_HIST == "YES") {
  ...
  # UNWEIGHTED: frac_no_wgt_{comm,low,high}
  ...
  # WEIGHTED (fraction-scaled):
  # comm uses frac_pop_wgt_comm, low/high use frac_risk_wgt_{low,high}
  hist_adj_wgt <- bind_rows(
    ...
    build_hist(
      risks      = c("low", "high"),
      df         = temp_dist,
      vars       = c("frac_risk_wgt"),
      weight_col = "frac_risk_wgt",
      mc.cores   = 2
    )
  )
  ...
  # UNWEIGHTED: no_wgt_{comm,low,high}
  hist_no_wgt <- build_hist(
    risks      = risk_list,
    df         = temp_dist,
    vars       = c("no_wgt"),
  ...
  # WEIGHTED (raw sums):
  # comm uses pop_wgt_comm, low/high use risk_wgt_{low,high}
  hist_adj_wgt <- bind_rows(
    build_hist(
      risks      = c("comm"),
      df         = temp_dist,
...
plot_temp_panels_risk <- function(data,
                                  hist_no_wgt,
                                  hist_adj_wgt,
                                  use_adj = TRUE) {
  ...
  make_strip <- function(r, is_leftmost = FALSE, tag_letter = "A") {
    ...
    rf_label_x <- x_limits[1] + 26
    
    # B/C pushed into the spacer to the left of their panel
    tag_x <- if (is_leftmost) 0.06 else -0.03
    
    # --- Top panel: response function ---
    ...
    # --- Spacer gap between RF and hist within a strip ---
  ...
  # --- Explicit spacer columns between panels (patchwork) ---
  spacer_width <- 0.03
  
  ((strips[[1]] | plot_spacer() | strips[[2]] | plot_spacer() | strips[[3]]) +
      plot_layout(widths = c(1, spacer_width, 1, spacer_width, 1))) +
...
print(plot_temp_panels_risk(data, hist_no_wgt, hist_adj_wgt, use_adj = FALSE))

# adjusted histogram
p <- plot_temp_panels_risk(data, hist_no_wgt, hist_adj_wgt, use_adj = TRUE)
