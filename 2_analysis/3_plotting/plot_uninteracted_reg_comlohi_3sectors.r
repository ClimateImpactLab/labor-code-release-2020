...
rf <- read_csv(glue(
  ...
  "uninteracted_reg_comlohi_full_response_2025_2dummy.csv"
))

# Temperature distribution and densities (global)
temp_dist <- read_csv(
  glue("{DIR_OUTPUT}/temp_dist/2_high_risk/global_temp_dist.csv")
)
...
plot_temp_panels <- function(data,
  ...
  header_labels <- c(
    "A All workers",
    "B Low-risk workers",
    "C Workers in Agri+Con",
    "D Workers in Manufacturing"
  )
