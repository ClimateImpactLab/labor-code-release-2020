...
foreach reg in $reg_list {
	...
	* differentiate treatment if reg is by risk
	if "`reg'" == "by_risk" loc reg_treatment (${vars_T_splines})##i.agri
	...
	* both regressions have interacted controls
	local reg_control (${usual_controls})##i.agri
	di "`reg_control'"
	
	* interact each fixed effect with the risk binary
	local reg_fe ""					
	foreach f in $`fe' {
		local reg_fe `reg_fe' `f'#i.agri
	}

	* set the ster file name and the notes to be included
	local ster_name "`reg_folder'/uninteracted_reg_`reg'_2025_agriandcomplement.ster"
