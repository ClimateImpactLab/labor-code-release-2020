...
gl dataset 		"/project/cil/battuta_shares/gcp/estimation/labor/code_release_int_data/regression_ready_data/labor_dataset_splines_nochn_tmax_chn_prev_week_no_ll_0_3sector_noWEU.dta"
...
foreach reg in $reg_list {
	...
	* differentiate treatment if reg is by risk
	if "`reg'" == "by_risk" loc reg_treatment  (${vars_T_splines})##i.manuf (${vars_T_splines})##i.high_risk3
	...
	* both regressions have interacted controls
	local reg_control (${usual_controls})##i.manuf (${usual_controls})##i.high_risk3
	di "`reg_control'"
	
	* interact each fixed effect with the risk categories
	local reg_fe ""					
	foreach f in $`fe' {
	    local reg_fe `reg_fe' `f'#i.manuf `f'#i.high_risk3
	}

	* set the ster file name and the notes to be included
	local ster_name "`reg_folder'/uninteracted_reg_`reg'_2025_manuf_noeu.ster"
	...
	* count regression N by risk category
	...
	count if included == 1 & manuf == 0 & high_risk3 == 0
	estadd scalar low_N = `r(N)'
	count if included == 1 & manuf == 1
	estadd scalar manuf_N = `r(N)'
	count if included == 1 & high_risk3 == 1
	estadd scalar high_risk3_N = `r(N)'
