*-------------------------------------------------------------------------------
* uninteracted_reg_comlohi_region_4sector.do
*-------------------------------------------------------------------------------
* PURPOSE
*   Estimate 4-sector labor-temperature regressions for each region
*
* STEPS
*   1. Loop over EuropeUS, LatinAmerica, and SouthAsia
*   2. Load the regional dataset with the chosen region knots
*   3. Set risk_level from sector2
...
log using "${DIR_OUTPUT}/logs/uninteracted_reg_comlohi_region_4sector.log", replace text
...
local reg_root "${DIR_STER}/uninteracted_reg_region_4sector"
...
foreach region of local regions {
    ...
    di as txt "Running 4-sector region regressions: `region'"
    ...
    confirm variable sector2

    * Europe has no detailed sector2 value
    quietly count if missing(sector2)
    if r(N) > 0 {
        di as txt "Dropping rows with missing sector2: " r(N)
        drop if missing(sector2)
    }

    cap drop risk_level
    gen byte risk_level = sector2
    assert inlist(risk_level, 0, 1, 2, 3)
    ...
    if inlist("`region'", "EuropeUS", "SouthAsia") {
    ...
    foreach spec of local reg_list {
        ...
        local ster_name "`reg_folder'/uninteracted_reg_`spec'_`region'_4sector.ster"
        local spec_desc "region=`region'; rcspl3 (`knots'); fe=$`fe_macro'; spec=`spec'; weight=`weight'; risk_level=sector2"
        ...
        estadd scalar manuf_transport_N = r(N)

        count if `included' == 1 & risk_level == 3
        estadd scalar construction_mining_N = r(N)
