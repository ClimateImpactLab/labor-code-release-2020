...
* log results
cap log close 
log using "${DIR_LOG}/interacted_poly_2factor_0205.smcl", replace

* select dataset and output folder
gl dataset      "${ROOT_INT_DATA}/regression_ready_data/labor_dataset_polynomials_nochn_tmax_chn_prev_week_no_ll_0_agnonag_272841_0203.dta"
...
// 		    *if "`reg'" == "1_factor" loc reg_treatment (${vars_T_splines} ${vars_T_x_gdp_splines})##i.risk_level
		...
		* -------------------------------------------------------------------------------------------------------
		    * set the ster file name and the notes to be included
		    ...
		    local spec_desc "polynomial, tmax, differentiated treatment with lr interaction, fe = $fe, reg_type = `reg'"
