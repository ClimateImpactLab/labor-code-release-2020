...
gl dataset 		"/project/cil/battuta_shares/gcp/estimation/labor/code_release_int_data/regression_ready_data/labor_dataset_splines_nochn_tmax_chn_prev_week_no_ll_0_3sector_agcoma.dta"
...
foreach reg in $reg_list {
	...
	* differentiate treatment if reg is by risk
	if "`reg'" == "by_risk" loc reg_treatment  (${vars_T_splines})##i.manuft (${vars_T_splines})##i.agri (${vars_T_splines})##i.connmine
	...
	* both regressions have interacted controls
	local reg_control (${usual_controls})##i.manuft (${usual_controls})##i.agri (${usual_controls})##i.connmine
	di "`reg_control'"
	
	* interact each fixed effect with the risk categories
	local reg_fe ""					
	foreach f in $`fe' {
	    local reg_fe `reg_fe' `f'#i.manuft `f'#i.agri `f'#i.connmine
	}

	* set the ster file name and the notes to be included
	local ster_name "`reg_folder'/uninteracted_reg_`reg'_2025_manuf_noeu_agcoma.ster"
	...
	* count regression N by risk category
	gen included = e(sample)
	count if included == 1 & manuft == 0 & agri == 0 & connmine == 0
	estadd scalar low_N = `r(N)'
	count if included == 1 & manuft == 1
	estadd scalar manuft_N = `r(N)'
	count if included == 1 & agri == 1
	estadd scalar agri_N = `r(N)'
	count if included == 1 & connmine == 1
	estadd scalar connmine_N = `r(N)'
