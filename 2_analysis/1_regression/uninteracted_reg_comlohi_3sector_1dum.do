...
run "${DIR_REPO_LABOR}/2_analysis/0_subroutines/function_test.do"
...
foreach reg in $reg_list {
	...
	* only include non-zero observations
	...
	keep if connmine == 1 | (manuf == 0 & connmine == 0)
	...
	* differentiate treatment if reg is by risk
	if "`reg'" == "by_risk" loc reg_treatment (${vars_T_splines})##i.connmine
	...
	* both regressions have interacted controls
	local reg_control (${usual_controls})##i.connmine
	di "`reg_control'"
	
	* interact each fixed effect with the risk binary
	local reg_fe ""					
	foreach f in $`fe' {
		local reg_fe `reg_fe' `f'#i.connmine
	}

	* set the ster file name and the notes to be included
	local ster_name "`reg_folder'/uninteracted_reg_`reg'_2025_connmine.ster"
	...
	* count regression N by risk
	...
	count if included == 1 & connmine == 0
