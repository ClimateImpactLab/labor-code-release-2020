...
#df <- fread("/project/cil/gcp/climate/_spatial_data/impactregions/weather_data/csv_daily/GMDF_tmax_temp_1950_to_2010_daily.csv")
...
opt_temp_l = 29.405891622251
opt_temp_h = 30.604584663960
...
dfb$f_h_opt_h <- opt_temp_h*dfb$HR_temp + ((opt_temp_h-27)^3)*dfb$HR_temp_s

dfb$f_l <- dfb$temp*dfb$LR_temp + dfb$temp_s*dfb$LR_temp_s
dfb$f_l_opt_l <- opt_temp_l*dfb$LR_temp + ((opt_temp_l-27)^3)*dfb$LR_temp_s
...
write.csv(key_irs, glue("/project/cil/home_dirs/egrenier/repos/labor-code-release-2020/disutility_ext/outputs/key_cities{suffix}.csv"))

#CARTOGRAPHY TIME
shp <- st_read("/project/cil/sacagawea_shares/gcp/regions/world-combo-new/agglomerated-world-new.shp", stringsAsFactors = FALSE) %>%
  st_set_crs(4326) %>%
  st_transform(
...
p = ggplot(data = effects) +
  ...
  scale_fill_gradientn(
    ...
    guide = guide_colorbar(title = "Hedonic value of thermal comfort in a low-risk job (% 2010 Income)",
...
print(p)
ggsave(glue("/project/cil/home_dirs/egrenier/repos/labor-code-release-2020/disutility_ext/outputs/Value_LR_job_map{suffix}.pdf"), p, bg = "white",width = 8, height = 6)
...
world_pw_mean <- sum(effects$pw_dis_diff, na.rm=T)
...
conts <- fread("/project/cil/gcp/regions/continents2.csv")
...
fwrite(aggregates,glue("/project/cil/home_dirs/egrenier/repos/labor-code-release-2020/disutility_ext/outputs/regional_aggregates{suffix}.csv"))
...
fwrite(pw_quants, glue("/project/cil/home_dirs/egrenier/repos/labor-code-release-2020/disutility_ext/outputs/pop_w_quantiles{suffix}.csv"))
