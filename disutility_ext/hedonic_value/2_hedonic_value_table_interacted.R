...
#' standard deviations using a parametric bootstrap over the coefficient VCV.
#'
#' INTERACTED MODEL: HR_temp and HR_temp_s are interacted with long-run mean
#' temperature (climtasmax) per IR. Effective HR coefficients therefore vary
#' by region, as does the per-region HR optimum T_opt_HR.
#'
#' CLIPPING: The HR curve is grounded at its own per-region optimum T_opt_HR.
#' The LR curve is grounded at the global LR optimum T_opt_LR and floored at
#' zero (losses only). d_h is clipped up to d_l: if HR is no worse than LR,
#' diff = 0.
#'
#' UNCERTAINTY: Parametric bootstrap — n_boot coefficient vectors are drawn
#' from the multivariate normal defined by the mean coefficients and the VCV.
#' The full damage calculation is run for each draw. Region-level and global
#' mean and SD are computed across draws.
#'
#==============================================================================#

# 0. Packages and paths ----

packages = c("data.table", "tidyverse", "sf", "scales", "glue", "purrr", "knitr", "kableExtra", "mvtnorm")
...
if (spec == "cold") {
  dfa    = dfa %>% filter(temp <= cutoff_temp)
  suffix = "_cold"
} else if (spec == "hot") {
  dfa    = dfa %>% filter(temp > cutoff_temp)
  suffix = "_hot"
...
#==============================================================================#
# 2. Coefs + variance covariance matrix ----
#==============================================================================#
...
# Sets f_hr'(T) = 0 and solves within each spline segment. 
# Second derivative test determines whether each root is a max or min;
...
find_T_opt_HR = function(ht, hs) {
  ...
  if (is.finite(disc) && disc >= 0 && a_q != 0) {
    for (T_cand in c((-b_q + sqrt(disc)) / (2*a_q), (-b_q - sqrt(disc)) / (2*a_q))) {
...
#==============================================================================#
# 5. merge weather data with socioeconomics and climate ----
#==============================================================================#
...
boot_region = matrix(NA, nrow = length(region_ids), ncol = n_boot, dimnames = list(region_ids, NULL))
boot_global = numeric(n_boot)
...
for (i in seq_len(n_boot)) {
  ...
  # Per-region HR effective coefficients and optima for this draw
  HR_eff_i = HR_t_i + HR_tc_i * dfa$climtasmax
  HR_s_eff_i = HR_s_i + HR_sc_i * dfa$climtasmax
  
  # T_opt_HR per region — computed on unique climtasmax values for speed
  clim_i = clim %>%
    mutate(
  ...
  # Clipping
  dfa_i$d_l = ifelse(dfa_i$f_l - f_l_opt_l_i < 0, dfa_i$f_l - f_l_opt_l_i, 0)
  ...
  # Store region-level results and global mean
  boot_region[as.character(dfb_i$hierid), i] = dfb_i$diff_dis_p
  boot_global[i] = weighted.mean(dfb_i$diff_dis_p, dfb_i$pop, na.rm = TRUE)
}
...
# Region-level mean and SD across draws
...
key_irs = regs %>%
  ...
  filter(!is.na(city)) %>%
  dplyr::select(city, diff_dis_p, SD)
