...
program define run_specification
	...
	if "`diff_treat'" == "diff_treat"{
		local treatment (`treatment')##i.high_risk_old
	}

	if "`diff_cont'" == "diff_cont"{
		local control (`control')##i.high_risk_old
		local fe 
		foreach f in ${`fe_ver'} {
			local fe `fe' `f'#high_risk_old
		}
	}
...
program define generate_grids
	...
	* add counts of observations, rep-units, and rep-unit-years.
	use "/project/cil/battuta_shares/gcp/estimation/labor/code_release_int_data/xtiles/rep_unit_terciles_count.dta", clear
...
program define generate_coef_polynomials
	...
	* compute coefficients for each variable at each polylnomial 
	...
	forval degree = 1/4{
		global b_temp_`degree'_hr _b[1.high_risk_old#c.tmax_p`degree'] 
		global b_temp_gdp_`degree'_hr _b[1.high_risk_old#c.tmax_p`degree'#c.log_gdp_pc_adm1]
		global b_temp_lrtmax_`degree'_hr _b[1.high_risk_old#c.tmax_p`degree'#c.lr_tmax_p1]

		forval lag = 1/6 {
			global b_temp_`degree'_hr ${b_temp_`degree'_hr}+_b[1.high_risk_old#c.tmax_p`degree'_v`lag']		
			global b_temp_gdp_`degree'_hr ${b_temp_gdp_`degree'_hr}+_b[1.high_risk_old#c.tmax_p`degree'_v`lag'#c.log_gdp_pc_adm1]
			global b_temp_lrtmax_`degree'_hr ${b_temp_lrtmax_`degree'_hr}+_b[1.high_risk_old#c.tmax_p`degree'_v`lag'#c.lr_tmax_p1]
		}
	}
			...
			foreach ref_temp in `center' `default_ref_temp' {	
				...
				* if treatment was not differentiated between high and low risk, gen RF for only low risk
				...
				foreach risk in `risk_list' {
					...
					if "`risk'" == "high" {
						loc risk_tag = "1.high_risk_old#c."
					}
					else{
						loc risk_tag = ""
					}
	...
	* lr: low risk, hr: high risk (risk interaction terms only), hl: lr + hr = response of high risk 
 	foreach risk in _lr _hr _hl {
 		* generate the 3 or 9 plots
 		...
 		if "`risk'" == "_hl" local plot_tag high_risk_old
 	...
 	if "`interaction'" == "triple_int" {
		...
		foreach marginal_var in `marg_vars' {
			* plot marginal effect of lrtmax and log gdp per capita
			...
			graph combine low_risk high_risk_old, plotregion(color(white)) graphregion(color(white)) cols(2) ycommon title("`marginal_var'") subtitle("`ster_name'")
