...
#    - Rows 4–5 are hard-coded pending the external SCC pipeline.
#
#  Data sources:
#    Row 1  col 1   : table4_hedonic_value.tex
#    Row 1  col 2-5 : run_hedonic(εH, εL)
#    Row 2  col 1-5 : run_disutility("rcp85", εH, εL)
#    Row 3  col 1-5 : run_disutility("rcp45", εH, εL)
#    Row 4-5 col 1-5: hard-coded from prior run
#==============================================================================#
...
#==============================================================================#
# 3. Rows 4–5: partial SCC — hard-coded pending SCC pipeline ----
#    Replace with computed values once run_disutility_scc_prep output is
#    fed through the discounting step.
#==============================================================================#

row4 <- list(
  col1 = list(mean = 12.1, p_lo = -0.3,  p_hi = 64.3),
  col2 = list(mean = 16.3, p_lo = -0.4,  p_hi = 86.9),
  col3 = list(mean =  8.6, p_lo = -0.2,  p_hi = 45.9),
  col4 = list(mean = 11.7, p_lo = -0.4,  p_hi = 59.5),
  col5 = list(mean = 12.4, p_lo = -0.7,  p_hi = 70.5)
)

row5 <- list(
  col1 = list(mean =  7.4, p_lo = -3.2,  p_hi = 47.0),
  col2 = list(mean =  9.9, p_lo = -4.3,  p_hi = 63.5),
  col3 = list(mean =  5.3, p_lo = -2.3,  p_hi = 33.5),
...
results_raw <- rbind(
  ...
  make_raw_df_frisch(row2, "disutility_rcp85"),
  make_raw_df_frisch(row3, "disutility_rcp45"),
  make_raw_df_frisch(row4, "scc_rcp85"),
  make_raw_df_frisch(row5, "scc_rcp45")
)
...
latex_table <- paste0(
  ...
  "\\toprule", NL,
  "& & \\multicolumn{5}{c}{\\textbf{Sensitivity to assumptions}}", CR, NL,
  "\\cmidrule(lr){3-7}", NL,
  "& & Baseline & \\multicolumn{4}{c}{Varying Frisch elasticity}", CR, NL,
