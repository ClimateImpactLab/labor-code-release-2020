...
# print(tail(results %>% filter(rcp=="rcp45") %>% select(pct_gdp, pct_gdp_old), n=50), n=50)
# print(tail(results %>% filter(rcp=="rcp45") %>% select(global_dmg_old, global_pop), n=50), n=50)
# print(tail(results %>% filter(rcp=="rcp45") %>% select(dmgpc_global, dmgpc_ce_global), n=50), n=50)

# write_csv(results, "/mnt/CIL_energy/code_release_data_pixel_interaction/intermediate_data/ce_damage_timeseries.csv")

# [ \sum_{i} ( damages_{i,t}*pop_{i,t} ) ] / globalgdp_{t}
...
plot_impact_timeseries = function(ssp, iam, adapt, risk, region, aggregation="", suffix="", output_folder = glue("{DIR_FIG}/mc/")){
  ...
  ce_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = global_ce_dmg)
  ce_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = global_ce_dmg)
  df_dmg_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = global_dmg)
  df_dmg_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = global_dmg)
  df_dmg_old_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = global_dmg_old)
  df_dmg_old_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = global_dmg_old)
  browser()
