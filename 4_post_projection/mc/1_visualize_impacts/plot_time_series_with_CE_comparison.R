...
plot_impact_timeseries = function(ssp, iam, adapt, risk, region, aggregation="", suffix="", output_folder = glue("{DIR_FIG}/mc/")){
  ...
  ce_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = pct_gdp_ce)
  ce_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = pct_gdp_ce)
  df_dmg_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = pct_gdp)
  df_dmg_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = pct_gdp)
  df_dmg_old_45 = df_ce %>% filter(rcp == "rcp45") %>% rename(mean = pct_gdp_old)
  df_dmg_old_85 = df_ce %>% filter(rcp == "rcp85") %>% rename(mean = pct_gdp_old)
