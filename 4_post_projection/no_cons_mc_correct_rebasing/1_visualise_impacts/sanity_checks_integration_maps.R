...
plot_impact_map = function(rcp, ssp, iam, adapt, year, risk, aggregation="", suffix="",output_folder = DIR_FIG){
  ...
  # browser()
  df= read_csv(glue('{ROOT_INT_DATA}/projection_outputs/extracted_data_mc_correct_rebasing_for_integration/{ssp}-{rcp}_{iam}_{risk}_{adapt}{aggregation}{suffix}_{year}_map.csv')) # browser()
...
output_folder_mc = paste0(DIR_FIG, "/sanity_checks_for_integration_newMC/")
for (ssp in c("SSP2","SSP3","SSP4")) {
  plot_impact_map(rcp="rcp85",ssp=ssp,iam="high", adapt="fulladapt",year=2099,risk="allrisk",aggregation="", output_folder = output_folder_mc)
