...
get_df_list_fig_2C = function(DB_data){
  ...
  load_df = function(rcp, adapt){
    ...
    df= read_csv(glue('{DB_data}/projection_outputs/extracted_data_mc_correct_rebasing_for_integration/SSP3-{rcp}_high_allrisk_{adapt}-wage-aggregated_global_timeseries.csv'))
...
plot_ts_fig_2C = function(output, DB_data){
  ...
  ggtitle(paste0("high", "-rcp85","-SSP3", "-fulluncertainty MC dollar value")) 
  print(paste0(output, "/mc_correct_rebasing_for_integration/diagnostics/fig", "_SSP3_fulluncertainty_time_series_wage.pdf"))
  ggsave(paste0(output, "/mc_correct_rebasing_for_integration/diagnostics/fig", "_SSP3_fulluncertainty_time_series_wage.pdf"), p)
  return(p)
}
