...
for (rcp in rcplist){
  for (ssp in ssplist){
    for (i in iam){
      ...
      ubcosts.b <- read.csv(paste0(inputwd, ssp, "-rcp45_low_rebased_fulladapt-gdp-aggregated_global_timeseries.csv"))
      ubcosts.b[,3:length(ubcosts.b)] <- ubcosts.b[,3:length(ubcosts.b)]*-100
      
      ubcosts.b.45 <- read.csv(paste0(inputwd, ssp, "-rcp45_low_rebased_fulladapt-gdp-aggregated_global_timeseries_interacted.csv")) 
