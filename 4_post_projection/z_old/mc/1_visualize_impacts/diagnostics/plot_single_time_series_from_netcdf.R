...
extract_csv <- function(adapt, single_dir) {
...
get_df_list_fig_2C = function(DB_data, model, single_dir){
  
  # Load in the impacts data: 
  load_df = function(adapt, model, single_dif){

    extract_csv(adapt, single_dir)
...
plot_ts_fig_2C = function(output, DB_data, model, single_dir){
  
  plot_df = get_df_list_fig_2C(DB_data = DB_data, model = model, single_dir = single_dir) 
  # browser()
...
output = DIR_FIG
p = plot_ts_fig_2C(output = output, 
  DB_data = DB_data, 
  single_dir = , 
  basename = )
