...
global chn_week_list  chn_next_week

global variables_list bins
...
foreach t_version in $t_version_list {
	foreach chn_week in $chn_week_list {	
		foreach data_ver in $data_ll_version_list {
			use "$final_path/labor_dataset_bins_`t_version'_`chn_week'_`data_ver'.dta", clear
			...
			* convert to 3C bins
			...
			save "$final_path/labor_dataset_bins_`t_version'_`chn_week'_`data_ver'_0Cto42C_3Cbins.dta", replace
		}
	}
}
