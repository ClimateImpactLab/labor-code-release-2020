...
program define merge_climate_data_file
	...
	* --- rename term0 -> t0 (only if exists) ---
	cap ds *term0*
	if !_rc {
	...
	* --- rename term1 -> t1 (only if exists) ---
	cap ds *term1*
	if !_rc {
	    foreach x of varlist `r(varlist)' {
	...
	* --- rename rcspline -> rcspl (only if exists) ---
	cap ds *rcspline*
	if !_rc {
	    foreach x of varlist `r(varlist)' {
...
foreach t_version in $t_version_list {
	foreach chn_week in $chn_week_list {	
		foreach variables in $variables_list {
			...
			*ds `t_version'_rcspl_3kn*27_28_41*
			*ds `t_version'_rcspl*27_28_41*t0*
