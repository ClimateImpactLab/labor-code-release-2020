...
if "${combine_surveys}" == "YES" {
	...
	* --------------------------------------------------
	* Scale mins_agwork the same way as mins_worked
	* IND is a daily diary so no scaling needed (same as mins_worked for IND)
	* Non-IND daily countries: multiply by sqrt(7)
	* BRA/MEX: already weekly, no scaling (will be filled in after this block)
	* --------------------------------------------------
	cap confirm variable mins_agwork
	if _rc == 0 {
		* variable exists (from IND), scale non-IND daily countries
		replace mins_agwork = mins_agwork * sqrt(7) if !inlist(iso, "CHN","BRA","MEX") & iso != "IND"
	}

	* --------------------------------------------------
	* Handle mins_agwork:
	* - IND: already has mins_agwork from cleaning script, scale it like mins_worked
	* - Other countries: mins_agwork = mins_worked if high_risk == 1, missing if high_risk == 0
	* --------------------------------------------------
	cap confirm variable mins_agwork
	if _rc != 0 {
		* mins_agwork not in data at all (IND data was not included or column missing)
		* generate for all observations
		gen mins_agwork = mins_worked if high_risk == 1
		replace mins_agwork = . if high_risk == 0
	}
	else {
		* mins_agwork exists from IND cleaning script
		* scale IND mins_agwork the same way as mins_worked (already weekly diary, no scaling needed)
		* for non-IND: fill in based on high_risk
		replace mins_agwork = mins_worked if high_risk == 1 & iso != "IND"
		replace mins_agwork = .           if high_risk == 0 & iso != "IND"
	}
