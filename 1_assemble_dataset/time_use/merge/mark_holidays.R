...
get_nth_weekday_v = function(years, month, weekday, n) {
	get_nth_weekday = function(month, year, weekday, n) {
		...
		ret = days[
			lubridate::wday(days, label=TRUE) == weekday & 
			day(days) <= max & 
			day(days) > min
			]
		return(ret)	
...
get_last_weekday_v = function(years, month, weekday) {	
	get_last_weekday = function(month, year, weekday) {
		...
		ret = days[lubridate::wday(days, label=TRUE) == weekday]
...
time_use = glue(
	'{ROOT_INT_DATA}',
	'/temp/all_time_use_pop_merged_reweighted_clustered_3sector_alt.dta') %>%
	read_dta() %>%
	data.table()
...
write.dta(final, glue(
	'{ROOT_INT_DATA}/temp/',
	'all_time_use_pop_merged_reweighted_clustered_holidays_marked_3sector_alt.dta'))
