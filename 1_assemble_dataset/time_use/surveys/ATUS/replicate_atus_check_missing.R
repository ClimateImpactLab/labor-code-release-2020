...
atussum = fread(glue("{input}/atussum_0314/atussum_0314.dat")) %>%
	...
	select(id, empstat, sex, age, total_mins, mins_worked, mins_not_worked, male, sex)
...
atusresp = fread(glue("{input}/atusresp_0314/atusresp_0314.dat")) %>%
	...
	#	) %>%
	mutate(
		high_risk = ifelse(industry %in% c(1, 2, 3, 4, 6), 1, 0)
		)
...
final = merge(atussum, atusresp, by=c("id"), all = TRUE) %>%
	merge(cps_all, by=c("id"), all = TRUE) %>%
	mutate(
	...
	select(
		ind_id, state, master_county_name, year, month, day, mins_worked, age, male, sex, hhsize, high_risk, sample_wgt, industry
		) 
...
#		'atus_replicated.csv')
#	)
fwrite(final, "/shares/gcp/estimation/labor/time_use_data/check_missing/USA_ATUS_time_use_w_missing.csv")
write.dta(final, "/shares/gcp/estimation/labor/time_use_data/check_missing/USA_ATUS_time_use_w_missing.dta")
