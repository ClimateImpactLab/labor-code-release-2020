...
input = glue("{DB}/GCP/WORKSHOP_STUDENT_FOLDERS")
...
gbr_comb = comb %>%
	filter(countrya == 37) %>%
	mutate(iso = "GBR") %>%
	merge(gbr, all.x=TRUE, by=c("countrya", "survey", "hldid", "persid"))

fra_comb = comb %>%
	filter(countrya == 12) %>%
	mutate(iso = "FRA") %>%
	merge(fra, all.x=TRUE, by=c("countrya", "survey", "swave", "msamp", "hldid", "persid"))
...
esp_comb = comb %>%
	filter(countrya == 34) %>%
	mutate(iso = "ESP") %>%
	merge(esp, all.x=TRUE, by=c("countrya", "survey", "swave", "msamp", "hldid", "persid"))# %>%
...
# removed "id"
comb_all = comb_geo %>%
	merge(ep, all = TRUE, by=c("countrya", "survey", "swave", "msamp", "hldid", "persid", "year","month","day"))
...
final = final %>% 
	select( # select the variables we want to write out into the estimating dataset
		iso, countrya, survey, hldid, persid, swave, msamp, cday, month, year, mins_worked, high_risk, occup, age, male,sex, hhsize, propwt, region
		) %>% 
	mutate(
	...
	select(
		iso, region_code, ind_id, year, month, day, mins_worked, high_risk, occup, age, male, sex,hhsize, sample_wgt
		)
...
write.dta(final, "/shares/gcp/estimation/labor/time_use_data/check_missing/EU_MTUS_time_use_w_missing.dta")
write.dta(final_gbr, "/shares/gcp/estimation/labor/time_use_data/check_missing/GBR_MTUS_time_use_w_missing.dta")
write.dta(final_fra, "/shares/gcp/estimation/labor/time_use_data/check_missing/FRA_MTUS_time_use_w_missing.dta")
write.dta(final_esp, "/shares/gcp/estimation/labor/time_use_data/check_missing/ESP_MTUS_time_use_w_missing.dta")
