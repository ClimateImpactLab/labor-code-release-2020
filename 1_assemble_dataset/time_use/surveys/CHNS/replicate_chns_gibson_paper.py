...
file_wages[file_wages['hours_worked_last_week'] < 0] = 0
...
# index: idind, wave
file_education = pd.read_stata(data_folder + "educ_00.dta")[['idind','wave','a11']]
file_education.columns = ['idind','wave','years_school']

# index: hhid, wave
file_assets = pd.read_stata(data_folder + "asset_00.dta")[['hhid','wave','l111','l116','l120','l121']]
file_assets.columns = ['hhid','wave','washing_machine_owned','fridge_owned','hh_own_ac','ac_owned']
...
# index: hhid, wave
file_hhinc = pd.read_stata(data_folder + "hhinc_pub_00.dta")[['hhid','wave','hhsize','hhinc']]
...
merged = merged.merge(file_education, on = ['idind','wave'], how = "outer", indicator = False)
merged = merged.merge(file_assets, on = ['hhid','wave'], how = "outer", indicator = False)

chns_new_data = merged
############################
#### additional merge ######
############################
...
chns_new_data = chns_new_data.assign(female = 1 - chns_new_data['male'] ) # generate male indicator
...
chns_new_data['farmer'] = np.where(chns_new_data.occupation_primary.isin([5]), 1, 0) # assign high risk
...
#chns_new_data.to_csv("/shares/gcp/estimation/labor/time_use_data/intermediate/CHN_CHNS_time_use_paper_replication.csv")
chns_new_data.to_stata("/shares/gcp/estimation/labor/time_use_data/intermediate/CHN_CHNS_time_use_paper_replication.dta")
