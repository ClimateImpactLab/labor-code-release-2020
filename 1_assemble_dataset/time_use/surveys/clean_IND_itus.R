...
b3.5 = read_dta(glue("{input}/Block-3-Item-5-Particulars-activity-selected-days-records.dta")) %>%
  ...
  dplyr::select(Key_membno, Key_hhold, day_type, time_spent, activity_code, is_work, occup_code)
...
b3_all = left_join(b3, b3.5, by=c('Key_membno', 'Key_hhold', 'day_type')) %>%
	...
	dplyr::summarize(
		...
		mins_worked_oc1 = sum(time_spent[is_work == 1 & occup_code ==1]), # ag related occupations
		mins_worked_oc2 = sum(time_spent[is_work == 1 & occup_code ==2]), # manufacturing etc.
		mins_worked_oc3 = sum(time_spent[is_work == 1 & occup_code ==3]), # mining + construction
