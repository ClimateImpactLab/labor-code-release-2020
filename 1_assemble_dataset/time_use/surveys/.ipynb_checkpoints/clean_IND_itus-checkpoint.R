...
b2 = read_dta(glue("{input}/Block-2-Particulars-Household-members-records.dta")) %>%
  ...
  dplyr::select( # select the datat we want and variables needed for merging (come back to this)
		Key_hhold, Key_membno, sex, age, male, high_risk_old, high_risk, high_risk3, manuf, self_emp, age #, age2,
		) %>%
...
final_dataset = all_geo %>% 
	...
	dplyr::select(
		st_name, district_name, year, month, day, ind_id, mins_worked, age, male, high_risk, high_risk2, high_risk3, manuf, self_emp, hhsize, sample_wgt
		) %>% 
	filter(
		year == 1999 | year == 1998,
		month >= 1 & month <= 12,
...
write.csv(final_dataset, glue("{ROOT_INT_DATA}/surveys/cleaned_country_data/IND_ITUS_time_use_3sector.csv"))
write.dta(final_dataset, glue("{ROOT_INT_DATA}/surveys/cleaned_country_data/IND_ITUS_time_use_3sector.dta"))
