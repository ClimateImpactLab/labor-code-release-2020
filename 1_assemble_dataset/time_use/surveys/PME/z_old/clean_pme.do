...
program define clean_file 
	...
	import delimited using "$data_folder/`filename'.csv", clear
	...
	*  9,120,422 missing in lodown
	*  5,134,007 missing in dta
	* another variable v429 (and v434 for those with a second job), what's the difference? - sum! 

	* age

	rename v234 age
	drop if age < 15 | age > 65
	* !!! there's negative age and 120 yrs old...
	...
	* if there's more days before the first sunday than after the last sunday, take the first week
	...
	drop v*
	gen age2 = age * age
	save "$data_folder/cleaned/cleaned_`filename'.dta", replace
...
clean_file pme_all_lodown
*clean_file pme_all_new
clean_file pme_all_new_from_dta
