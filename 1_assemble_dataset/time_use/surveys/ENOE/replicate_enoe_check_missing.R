...
outcome = merge(
	...
	merge(
		vivienda,
		by=c('CD_A', 'ENT', 'CON', 'V_SEL', 'N_PRO_VIV', 'N_ENT', 'UPM'), all = TRUE
		) %>%
	merge(
		ca,
		by=c('CD_A', 'ENT', 'CON', 'V_SEL', 'N_HOG', 'H_MUD', 'N_PRO_VIV', 
			'N_ENT', 'N_REN', 'UPM', 'PER'), all = TRUE
		) %>%	mutate(
		year = year + 2000
		) %>% 
		mutate(
...
enoe_geo = merge(enoe_geo, enoe_cw, by=c('state', 'municipality'), all=TRUE)
...
cw = merge(enoe_geo, shp, by=c('state_name', 'municipality_name'), all=TRUE)

# now merge in the outcome data 	
final = outcome %>%
	merge(cw, by=c('state', 'municipality'), all=TRUE) %>%
	rename(
		ind_id = id
		) %>% 
	select(
		ind_id, state_name, municipality_name, 
		prev_sunday, mins_worked, male, age, high_risk, hhsize, industry,
		sample_wgt
		) %>% 
	mutate(
...
fwrite(final, "/shares/gcp/estimation/labor/time_use_data/check_missing/MEX_ENOE_time_use_w_missing.csv")
write.dta(final, "/shares/gcp/estimation/labor/time_use_data/check_missing/MEX_ENOE_time_use_w_missing.dta")
