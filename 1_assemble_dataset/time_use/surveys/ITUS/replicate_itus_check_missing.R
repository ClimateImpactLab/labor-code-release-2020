...
b2 = read_dta(glue("{input}/Block-2-Particulars-Household-members-records.dta")) %>%
	...
	select( # select the datat we want and variables needed for merging (come back to this)
		Key_hhold, Key_membno, sex, age, male, high_risk_old, high_risk, industry #age, age2,
		) %>%
...
final_dataset = all_geo %>% 
	...
	dplyr::select(
		st_name, district_name, year, month, day, ind_id, mins_worked, age, sex, male, high_risk, hhsize, sample_wgt, industry
		) 
	#%>% 
#	filter(
#		year == 1999 | year == 1998,
...
write.csv(final_dataset, "/shares/gcp/estimation/labor/time_use_data/check_missing/IND_ITUS_time_use_w_missing.csv")
write.dta(final_dataset, "/shares/gcp/estimation/labor/time_use_data/check_missing/IND_ITUS_time_use_w_missing.dta")
