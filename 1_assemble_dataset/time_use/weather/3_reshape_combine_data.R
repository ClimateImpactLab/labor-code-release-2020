...
get_transf <- function(t_version){
  ...
  transformations <- list(bins=bins, polynomials=polynomials, prcp=prcp, splines_nochn = splines_nochn, splines_wchn = splines_wchn, yearly = yearly)
...
reshape <- function(ctry, transf, climate_source, admin){
...
t_version_list = c('tmax')
countries <- vector(mode = "list", length = 9)
###### use the correct set of countries! for no chn splines use second line
names(countries) <- c('GBR','FRA','ESP','IND','CHN','USA','MEX','BRA','WORLD')
...
countries$MEX <- "adm1"
countries$BRA <- "adm1"
countries$WORLD <- "adm0"

transf_columns_yearly <- c("yearly")
...
for (country in names(countries)) {
  for (t in t_version_list) {
    ...
    # reshaping
    args = expand.grid(climate_source=climate_source,ctry=country, transf=unlist(transf_list[names(transf_list) %in% transf_columns_yearly]), admin=countries[country]) 
    ...
    # # combining
    args = expand.grid(climate_source=climate_source,ctry=country, var=transf_columns_yearly, admin=countries[country], t_version=t)
    ...
    # renaming
    rename_yearly(climate_source=climate_source,ctry=country, admin=countries[country], t_version=t)
  }
}
