...
get_transf <- function(t_version){
  ...
  bins <- c() 
  polynomials <- c()
  polynomials_above_below <- c()
  prcp <- c()
  splines <- c()
  splines_nochn <- c()
  splines_nochn_best <- c()
  ...
  for (type in c("Above")){
    for (k in c(1,2,3,4)){
      polyname <- glue("{t_version}_poly{type}0_{k}")
      polynomials_above_below <- c(polynomials_above_below, polyname)
    }
  }

  # for (type in c("Above", "Below")){
  #   for (k in c(1,2,3,4)){
  #     polyname <- glue("{t_version}_poly{type}27_{k}")
  #     polynomials_above_below <- c(polynomials_above_below, polyname)
  #   }
  # }
  ...
  for (k in c(3,4)){
    for (i in seq(0, k-2, 1)){
      splines <- c(splines, glue("{t_version}_rcspline_wchn_{k}kn_old_term{i}"))
      splines_nochn <- c(splines_nochn, glue("{t_version}_rcspline_nochn_{k}kn_old_term{i}"))
  ...
  for (k in c(3)){
    for (i in seq(0, k-2, 1)){
      splines_nochn_best <- c(splines_nochn_best, glue("{t_version}_rcspline_nochn_best_{k}kn_old_term{i}"))
...
reshape <- function(ctry, transf, climate_source, admin, yearly){
  ...
  if (yearly==FALSE){
    ...
    # nrow(unique(dt[,.(adm3_id, year,month,day)]))
    # nrow(dt[,.(adm3_id, year,month,day)])
...
print("var")
print(var)
  var = ifelse(var %in% c("polynomials", "splines", "splines_nochn", "splines_nochn_best", "polynomials_above_below", "bins"), glue("{t_version}_{var}"), glue("{var}"))
print(var)
  print("here")
  # print(var)
  # message(var)
...
rename <- function(ctry, climate_source, admin, t_version='tmax', rename_long_run, rename_splines, rename_splines_nochn,rename_splines_nochn_best, rename_precip, rename_polynomials, rename_bins, rename_below0){

  print(glue("renaming {ctry}"))
  paths = paths(ctry=ctry,admin=admin)

  polynomials_new <- c()
  polynomials_above_below_new <- c()
  prcp_new <- c()
  splines_new <- c()
  splines_nochn_new <- c()
  splines_nochn_best_new <- c()
  
  long_run_new <- c()

  for (type in c("above")){
    for (k in c(1,2,3,4)){
      polyname <- glue("{t_version}_{type}0_p{k}")
      polynomials_above_below_new <- c(polynomials_above_below_new, polyname)
    }
  }

  # for (type in c("above", "below")){
  #   for (k in c(1,2,3,4)){
  #     polyname <- glue("{t_version}_{type}27_p{k}")
  #     polynomials_above_below_new <- c(polynomials_above_below_new, polyname)
  #   }
  # }
  ...
  for (k in c(3,4)){
    for (i in seq(0, k-2, 1)){
      splines_new <- c(splines_new, glue("{t_version}_rcspl_{k}kn_t{i}"))
      splines_nochn_new <- c(splines_nochn_new, glue("{t_version}_rcspl_nochn_{k}kn_t{i}"))
  ...
  for (k in c(3)){
    for (i in seq(0, k-2, 1)){
      splines_nochn_best_new <- c(splines_nochn_best_new, glue("{t_version}_rcspl_nochn_best_{k}kn_t{i}"))
  ...
  if (rename_splines==TRUE) {
    splines_dt <- fread(glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_splines_{admin}.csv"))
    setnames(splines_dt, old=transf_list$splines, new=splines_new)
    haven::write_dta(splines_dt, glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_splines_{admin}.dta"))
  ...
  if (rename_splines_nochn_best==TRUE) {
    splines_nochn_best_dt <- fread(glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_splines_nochn_best_{admin}.csv"))
    setnames(splines_nochn_best_dt, old=transf_list$splines_nochn_best, new=splines_nochn_best_new)
    haven::write_dta(splines_nochn_best_dt, glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_splines_nochn_best_{admin}.dta"))
  ...
  if (rename_below0==TRUE) {
    below_0_bin <- fread(glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_below_zero_bin_{admin}.csv"))
    haven::write_dta(below_0_bin, glue("{paths$outputdir}/{climate_source}_{ctry}_{t_version}_below_zero_bin_{admin}.dta"))    
  }
...
# transf_columns <- c("splines_nochn"，"bins")
...
for (country in names(countries)) {
  for (t in t_version_list) {
    ...
    mcmapply(FUN=reshape, ctry=args$ctry, transf=args$transf, climate_source=args$climate_source, admin=args$admin, yearly=args$yearly, mc.cores=24)
    ...
    mcmapply(combine, ctry=args$ctry, climate_source=args$climate_source, var=args$var, admin=args$admin, t_version=args$t, mc.cores=1)

    # renaming
    rename(climate_source=climate_source,ctry=country, rename_splines=FALSE, rename_splines_nochn=FALSE,rename_splines_nochn_best=FALSE, rename_precip=FALSE, rename_polynomials=FALSE, rename_long_run=FALSE, rename_bins=TRUE, rename_below0=FALSE,admin=admin, t_version=t)
  }
}
