...
emp = read_csv(glue("{lab}/adm1_empshares.csv")) %>%
	select(year, geolev1, geolev1_pop,total_pop, ind_highrisk_share, ind_highrisk_share_no50)
...
clim = read_csv(glue("{SHARES}/estimation/labor/employment_shares_data/climate_data/2_intermediate/GMFD_adm1_internal_new.csv")) %>%
...
# save
write_csv(all, glue("{lab}/emp_inc_clim_merged.csv"))
